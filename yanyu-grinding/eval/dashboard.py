...
CLI：
    python3 -m yanyu_eval.dashboard --session session.json --output dashboard.png
    python3 -m yanyu_eval.dashboard --session session.json --output dashboard.png --mock
"""
...
def render(session: dict[str, Any], output: str) -> None:
    ...
    fig.suptitle(
        f"砚友研磨 · 对话质量 Dashboard  ({session.get('topic','(未指定主题)')})",
        fontsize=14,
        fontweight="bold",
    )
