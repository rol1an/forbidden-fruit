...
> **何时读**：首次运行 `scripts/profile_init.py` 创建 Bitable 表时；改 schema 时；调试 `profile_read.py` / `profile_write.py` 时。
...
| `last_session_at` | 日期时间 | 上次研磨时间 |
| `session_count` | number | 累计研磨次数 |
| `domain_familiarity_tags` | 多值 select | 用户已涉足的领域标签（供 yanyu-fusion 选已知 × 未知碰撞）|
