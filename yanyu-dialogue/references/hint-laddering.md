...
### L2 · Recall trigger（反直觉锚点 + 形状提示）

**给什么**：①**反直觉锚点**（用户脑中默认答案 + agent 反向暗示）；②**形状提示**（字母数 / 词性 / 缩写形式）。
**不给什么**：术语名、定义、推理结论。

**关键约束**：agent 必须**先在内心反推用户脑中的默认答案是什么**，写入 `turns[].inferred_user_default` 字段供 judge.py 抽查。如果默认答案推不出来 → **退回 L1，不要给 L2**（盲猜的反直觉锚点会脱靶）。

**MathDial 打标**：`focus`（没给名字，不算 telling）；`ladder_level: 2`

**模板（5 条）**

| 用户默认答案 | L2 话术 |
|---|---|
| 用户以为是 "embedding 模型" | "反直觉之处在于**不是 embedding 模型**，而是另一个机制——英文缩写 3 个字母。" |
| 用户以为是 chunking 大小 | "瓶颈不在 chunk 大小，在 chunk 的**边界**——一个 6 个字母的英文词。" |
| 用户以为是 cosine similarity | "排序不靠它，靠另一个——名字以 **L** 开头。" |
| 用户以为是 fine-tune | "答案是 fine-tune 的反面——一个用户**侧**做的操作。" |
| 用户以为是数据量 | "瓶颈不在数据量，在数据**采集**的某个特定环节——这个环节英文是动词。" |
